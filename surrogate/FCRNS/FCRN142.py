...
class Model(nn.Module):
    def __init__(self):
        ...
        self.conv2 = torch.nn.Conv2d(4, 4, (3, 3))
        self.conv3 = torch.nn.Conv2d(4, 4, (2, 2))
        self.conv4 = torch.nn.Conv2d(4, 4, (1, 1))

        self.pool1 = torch.nn.MaxPool2d((2, 6))
        self.pool2 = torch.nn.MaxPool2d((1, 3))
        self.pool3 = torch.nn.MaxPool2d((1, 2))
    ...
    def forward(self, x):
        ...
        x3_p = self.pool3(x3)
        x4 = self.conv3(x3_p).view(x.shape[0], -1)
        # output = x3.max(dim=1)[0]
        output = torch.mean(x4, dim=1)
        return output
